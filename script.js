window.onload = () => {
    ...
    function generateFretboard() {
        ...
        for (let i = 0; i < numStrings; i++) {
            ...
            string.style.top = `${((numStrings - 1 - i) * 40) + 20}px`;
        ...
        for (let s = 0; s < numStrings; s++) {
            for (let f = startFret; f <= endFret; f++) {
                ...
                notePosition.dataset.string = numStrings - s;
                notePosition.dataset.fret = f;
                notePosition.style.top = `${((numStrings - 1 - s) * 40) + 20}px`;
                if (f === 0) {
                    notePosition.style.left = `${((f - startFret) * 60)}px`;
                } else {
    ...
    function updateDisplay() {
        ...
        positions.forEach(pos => {
            ...
            if (noteDiv) {
                ...
                dot.style.backgroundColor = pos.color;
    ...
    function parsePositions() {
        ...
        return positionsText.split('\n').map(line => {
            ...
            const string = parseInt(parts[0].trim());
            const fret = parseInt(parts[1].trim());
            const color = parts[2] ? parts[2].trim() : 'red';
            return { string, fret, color };
        });
    }
    ...
    function calculateNote(stringNum, fret) {
        const tuningInputs = document.querySelectorAll('.tuning-input');
        const tuning = Array.from(tuningInputs).map(input => parseInt(input.value));
        const openNoteValue = tuning[6 - stringNum];
    ...
    function getNoteText(noteValue) {
        ...
        if (rootNote === -1) {
            return NOTES_SHARP[noteValue];
        } else {
            const interval = (noteValue - rootNote + 12) % 12;
            const intervalSelect = document.querySelector(`.interval-select[data-interval='${interval}']`);
            if (intervalSelect) {
                return intervalSelect.value;
            }
            return INTERVALS[interval]; // Fallback for intervals without a dropdown
        }
    }
    ...
    startFretInput.addEventListener('change', updateDisplay);
    endFretInput.addEventListener('change', updateDisplay);

    rootNoteSelect.addEventListener('change', () => {
        const intervalControls = document.getElementById('interval-notation-controls');
        if (rootNoteSelect.value === '-1') {
    ...
    const intervalSelects = document.querySelectorAll('.interval-select');
    intervalSelects.forEach(select => select.addEventListener('change', updateDisplay));

    commentPreview.style.display = 'none';

    commentInput.addEventListener('blur', () => {
        commentPreview.innerHTML = marked(commentInput.value);
        commentInput.style.display = 'none';
        commentPreview.style.display = 'block';
    });

    commentPreview.addEventListener('click', () => {
